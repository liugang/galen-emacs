			...
			if (index < MAX_P2P_GROUP_SIZE)
			{
				...
				else if (bBeacon == FALSE)
				{
					...
					if (pAd->MlmeAux.Channel)
						pAd->P2pTable.Client[index].ListenChannel = pAd->MlmeAux.Channel;
...
/*	IN BOOLEAN	bSendRsp)  */
{
		...
		DBGPRINT(RT_DEBUG_ERROR, ("======  PeerP2pProbeReq(), line %d  here \n", __LINE__));
