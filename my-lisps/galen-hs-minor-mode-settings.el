(defun galen-func/hs-minor-mode-face-settings ()
...
(eval-after-load "hideshow"
  `(galen-func/hs-minor-mode-face-settings))
...
(defun galen-func/hs-minor-mode-settings ()
  ...
  (setq-default mode-line-format
                (append '((:eval (galen-func/hs-display-headline))) mode-line-format))

  (setq hs-set-up-overlay 'galen-func/hs-abstract-overlay)
  ...
  (defun galen-func/hs-abstract-overlay (ov)
  ...
  (defun galen-func/hs-toggle-hiding-all ()
    "Toggle hideshow all."
    (interactive)
  ...
  (defun galen-func/toggle-fold-all ()
    "Toggle fold all."
    (interactive)
    (if fold-all-fun
        (call-interactively fold-all-fun)
      (galen-func/hs-toggle-hiding-all)))

  (defun galen-func/toggle-fold ()
    "Toggle fold."
    (interactive)
    (if fold-fun
  ...
  (defun hs-minor-mode-4-emaci-settings ()
    ...
    (eal-define-keys
     'emaci-mode-map
     `(("s" galen-func/toggle-fold)
       ("S" galen-func/toggle-fold-all))))
...
(provide 'galen-hs-minor-mode-settings)
