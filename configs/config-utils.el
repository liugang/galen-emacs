...
(defun run-current-file ()
  (interactive)
;  (setq full_name (buffer-file-name))
;  (setq file_name (file-name-nondirectory full_name))
  (setq file_name (buffer-file-name))
  (setq file_ext (file-name-extension file_name))
  (setq file_pre (file-name-sans-extension file_name))
  (setq prog_name
    (cond ((string= file_ext "py")  "python")
          ((string= file_ext "sh")  "bash")
          ((string= file_ext "c")   "gcc")
          ((string= file_ext "cpp") "g++")
          ((string= file_ext "el")  "lisp")
          (t "")))
...
;  (message cmd_str)
  ...
  (unless (string= prog_name "")
    (progn
      (if (or (string= prog_name "gcc") (string= prog_name "g++"))
      (setq cmd_str (format "%s -g -o %s %s" prog_name file_pre file_name))
    (setq cmd_str (format "%s %s" prog_name file_name)))
      (message cmd_str)
;      (shell-command "eshell")
      (eshell-command cmd_str)
