		    ...
		    wireless_config *	info)
{
  ...
  if(iw_get_ext(skfd, ifname, SIOCGIWFREQ, &wrq) >= 0)
    {
      info->has_freq = 1;
      info->freq = iw_freq2float2(&(wrq.u.freq));
 ...
 */
double
iw_freq2float2(const iwfreq *	in)
{
#ifdef WE_NOLIBM
  /* Version without libm : slower */
  int		i;
		   ...
		   const struct iw_range *	range)
{
  ...
  for(k = 0; k < range->num_frequency; k++)
    {
      ref_freq = iw_freq2float2(&(range->freq[k]));
      if(freq == ref_freq)
		   ...
		   const struct iw_range *	range)
{
  ...
  for(k = 0; k < range->num_frequency; k++)
	...
	{
	  *pfreq = iw_freq2float2(&(range->freq[k]));
	  return(channel);
	}
    }
  /* Not found */
 ...
 */
void
iw_ether_ntop2(const struct ether_addr *	eth,
	      char *			buf)
{
...
#ifdef DEBUG
	  char buf[20];
	  iw_ether_ntop2((const struct ether_addr *) mac, buf);
			  ...
			  struct wireless_scan *	wscan)
{
  ...
  switch(event->cmd)
    ...
    case SIOCGIWFREQ:
      wscan->b.has_freq = 1;
      wscan->b.freq = iw_freq2float2(&(event->u.freq));
